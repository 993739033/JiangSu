 ...
 */
public class HarmlessMessageConfirmCollectionActivityPresenter extends BasePresenter<IHarmlessMessageConfirmCollectionActivity> {

    private String result;

    public HarmlessMessageConfirmCollectionActivityPresenter(IHarmlessMessageConfirmCollectionActivity view) {
        super(view);
    }

    public void getPageDetil(String fstId) {
        addSubscribe(NetClient.getHarmlessListDetil(getUserId(), "CK3", fstId)
     ...
     * 上传
     */
    public void upload(UploadSickDeadAnimBean bean, String uuid,String uuid2) {
//        addSubscribe(NetClient.uplaod(new Gson().toJson(bean), getUserId(), "APP_BSDWRK",null)
        if (getLock().isLock()) return;
        getLock().lock();
        addSubscribe(NetClient.uplaod(new Gson().toJson(bean), "APP_BSDWRK", getUserId(), uuid)
                .map(baseMsgBean -> {
                    if (baseMsgBean.getErrorCode() != 0) {
                        ...
                        for (File file : PhotoUtils.getWuHaiHuaDir().listFiles()) {
                            if (file.getName().contains(uuid)||file.getName().contains(uuid2)) {
                                //上传
