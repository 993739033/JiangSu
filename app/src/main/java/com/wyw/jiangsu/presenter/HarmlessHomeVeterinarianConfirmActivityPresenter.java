 ...
 */
public class HarmlessHomeVeterinarianConfirmActivityPresenter extends BasePresenter<IHarmlessHomeVeterinarianConfirmActivity> {

    public HarmlessHomeVeterinarianConfirmActivityPresenter(IHarmlessHomeVeterinarianConfirmActivity view) {
        super(view);
    }

    public void getPageDetil(String fstId) {
        addSubscribe(NetClient.getHarmlessListDetil(getUserId(), "CK5", fstId)
     ...
     * 上传
     */
    public void upload(UploadHomeConfimBean bean, String uuid) {
        if (getLock().isLock())return;
        getLock().lock();
        addSubscribe(NetClient.uplaod(new Gson().toJson(bean), "APP_ZCSYQR", getUserId(), uuid)
                .map(baseMsgBean -> {
