...
public class MilkStationAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MilkStationAdapter(Context context, List<BreedingRecordData.DataListBean> list, View headerView) {
...
*/
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_TYPE.ITEM_HEAD.ordinal()) {
            ...
            return new ListviewViewHolder(mLayoutInflater.inflate(R.layout.milk_adapter, parent, false));
        }
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof ListviewViewHolder) {
            ...
            ((ListviewViewHolder) holder).tvJiludanBianhao.setText(listBean.get(position - 1).getFCityAdd());//getFCityAdd());
            ((ListviewViewHolder) holder).tvJianyiTime.setText(listBean.get(position - 1).getFCategory());//getFCategory());
            ((ListviewViewHolder) holder).tvHouzhuName.setText(listBean.get(position - 1).getClsl());//getClsl());
            ((ListviewViewHolder) holder).tvAnimalType.setText(listBean.get(position - 1).getFLegalPerson());//getFLegalPerson());
            ((ListviewViewHolder) holder).tvQuantityDanwei.setText(listBean.get(position - 1).getFPhone());//getFPhone());
        }
    }
