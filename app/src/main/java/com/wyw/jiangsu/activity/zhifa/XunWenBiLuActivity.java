...
import com.wyw.jiangsu.activity.model.ConversationModel;
...
import com.wyw.jiangsu.presenter.XunWenBiLuActivityPresenter;
...
public class XunWenBiLuActivity extends MVPActivity<XunWenBiLuActivityPresenter> implements IXunWenBiLuActivity {
...
//    Spinner spinnerEndFen;
    ...
    @BindView(R.id.bt_xunwen_renyuan)
    Button btXunwenRenyuan;
...
//    @BindView(R.id.sp_beixunwen_sex)
//    Spinner spBeixunwenSex;
    @BindView(R.id.rg_sex_type)
    RadioGroup rg_sex_type;
    ...
    Button btBaocun;
    //要上传的实体类
    private XunWenBiLuBean bean = new XunWenBiLuBean();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layout_xunwen_bilu);
        ButterKnife.bind(this);
    }

    @Override
    protected XunWenBiLuActivityPresenter createPresenter() {
        return new XunWenBiLuActivityPresenter(this);
    }
    ...
    public void bindData() {
        setTitle("询问笔录");
        setTime();
//        spBeixunwenSex.setAdapter(new ArrayAdapter<>(this, R.layout.spinner_simple, R.id.tv,
//                getResources().getStringArray(R.array.sex)));
        conversationModel = new ConversationModel(this, linConversation);
        initDialog();
    }
            ...
            public void onClick(View v) {
                hmSelectDialog.setmTimePickListener1(new HourMinutePickDialog.TimePickListener1() {
                    ...
                    public void getTime(String hour, String minute) {
                        ...
                        hmfrom[1] = minute;
                        if (!checkTime()) {
                            Toast.makeText(XunWenBiLuActivity.this, "终止时间不能大于初始时间", Toast.LENGTH_SHORT).show();
                            hmfrom[0] = tempHour;
                            hmfrom[1] = tempMinute;
                            return;
                        }
...
//        spinnerEndFen.setSelection(OtherUtil.getCurrentMinutePosition());
    ...
    public void bindListener() {
        ...
        //询问人选择
        btXunwenRenyuan.setOnClickListener(v -> {
...
//        });
    ...
    private void showNormalDialog(){
        final AlertDialog.Builder dialog =
                new AlertDialog.Builder(XunWenBiLuActivity.this);
...
//        ((ArrayAdapter) spinnerDay.getAdapter()).notifyDataSetChanged();
        ...
        if (requestCode == Constance.ACTIVITY_CODE) {
            if (resultCode == RESULT_OK) {
                ...
                if (list != null && list.size() == 2) {
                    etXunwenRenyuan1.setText(list.get(0).getFname());
                    etXunwenRenyuan2.setText(list.get(1).getFname());
        ...
        if (requestCode == Constance.ACTIVITY_CODE + 1) {
            if (resultCode == RESULT_OK) {
                ...
                if (databean != null) {
                    ...
                    etXunwenRenyuan1.setText(s1);
                    etXunwenRenyuan2.setText(s2);
    ...
    private XunWenBiLuBean getData() {
        bean = new XunWenBiLuBean();
...
//        bean.setFsex(spBeixunwenSex.getSelectedItem().toString());  //性别
        if (rg_sex_type.getCheckedRadioButtonId()==R.id.rb_male){
            bean.setFsex("男");  //性别
        }else {
            bean.setFsex("女");  //性别
        }
...
//        bean.setFfengz1(spinnerEndFen.getSelectedItem().toString());//分钟2
    ...
    private boolean check() {
        ...
        if (TextUtils.isEmpty(etXunwenDidian.getText().toString())) {
            showToast("请填写询问地点");
            return false;
        }
        ...
        if (TextUtils.isEmpty(etBeixunwenName.getText().toString())) {
            showToast("请填写被询问人姓名");
            return false;
        }
        if (TextUtils.isEmpty(etBeixunwenAge.getText().toString())) {
            showToast("请填写被询问人年龄");
            return false;
        }
        if (TextUtils.isEmpty(etBeixunwenPhone.getText().toString())) {
            showToast("请填写被询问人电话");
            return false;
        }
        if (TextUtils.isEmpty(etBeixunwenId.getText().toString())) {
            showToast("请填写被询问人身份证号");
            return false;
        }
        if (TextUtils.isEmpty(etBeixunwenDanwei.getText().toString())) {
            showToast("请填写被询问人工作单位");
            return false;
        }
        if (TextUtils.isEmpty(etBeixunwenWork.getText().toString())) {
            showToast("请填写被询问人职务");
            return false;
        }
        if (TextUtils.isEmpty(etBeixunwenAdress.getText().toString())) {
            showToast("请填写被询问人住址");
            return false;
        }
        if (TextUtils.isEmpty(etZhifaRenyuan.getText().toString())) {
            showToast("请填写执法证件");
            return false;
        }
        if (TextUtils.isEmpty(etXunwenBilu.getText().toString())) {
            showToast("请回答");
            return false;
        }
        if (conversationModel.check()) {
            return false;
        }
        return true;
    }
