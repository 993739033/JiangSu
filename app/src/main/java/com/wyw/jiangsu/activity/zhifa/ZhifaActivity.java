 ...
 * 执法总览模块
 */
public class ZhifaActivity extends MVPActivity<HarmlessFlowActivityPresenter> implements IHarmlessFlowActivity, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void bindListener() {
        ...
        RecyclerItemClickSupport.addTo(recycler).setOnItemClickListener((recyclerView, position, v) -> {
            ...
            switch (data.getFsm1()) {
                //添加案件登记
                case "50":
                    startActivity(new Intent(getContext(), AddCaseRegisterActivity.class));
                    break;
                // 添加出具当场处罚决定书
                case "51":
                    startActivity(new Intent(getContext(), AddPunishmentBookActivity.class));
                    break;
                //现场检查(勘验)笔录
                case "67":
                    startActivity(new Intent(getContext(), XianChangCheckBiLuActivity.class));
                    break;
                //询问笔录
                case "68":
                    startActivity(new Intent(getContext(), XunWenBiLuActivity.class));
                    break;
            }
        });
    }
