...
import com.wyw.jiangsu.presenter.AnimalQuarantineActivityPresenter;
...
public class AnimalQuarantineActivity extends MVPActivity<AnimalQuarantineActivityPresenter> implements IAnimalQuarantineActivity, SpringView.OnFreshListener {
    ...
    private AnimalQuarantineActivityBean.DataListBean dataListBean = new AnimalQuarantineActivityBean.DataListBean();
    ...
    @Override
    protected AnimalQuarantineActivityPresenter createPresenter() {
        return new AnimalQuarantineActivityPresenter(this);
    }

    @Override
    public void bindData() {
        tvTitle.setText("动物检疫申报单查询列表");
...
//        adapter = new QueryRecylerAdapter(this, getDataa());
        adapter = new DeclareAdapter(this, new ArrayList<>(), headerView.getHeaderView());
...
//        getPresenter().search();
    ...
    public void bindListener() {
        ...
        RecyclerItemClickSupport.addTo(mRecyclerView).setOnItemClickListener((recyclerView, position, v) -> {
            if (position != 0) {
                Intent intent = new Intent(this, AnimalQuarantineDetilActivity.class);
...
//    }
    ...
    @Override
    public void addListData(List<AnimalQuarantineActivityBean.DataListBean> list) {
        adapter.addAll(list);
    }

    @Override
    public void setData(List<AnimalQuarantineActivityBean.DataListBean> list) {
        adapter.setmDatas(list);
    }
}
